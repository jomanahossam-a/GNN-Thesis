...
class Training:
    ...
    def train(self, games, solutions):
        ...
        self._network = ParityGameGraphSAGENetwork(35, 35, 10)  # Instantiate the network

        #self._network.load_state_dict(torch.load('GAT_pg_solver_20220914.pth'))  # Load pretrained weights
...
class Predictor:
    ...
    def predict(self, games):
        ...
        if isinstance(self._output, str):
            with open(self._output, 'w') as f, open('prediction_time.csv', 'w', newline='') as csvfile:
            ...
            with open('prediction_time.csv', 'w', newline='') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(['Game Path', 'Elapsed Time (s)'])
                for input in games:
                    print("Predicting path:", input)
    ...
    def _predict_single(self, input, output, identifier=None):
        ...
        predictions = [f"Node {node}: {'True' if out_nodes[node][0].item() > out_nodes[node][1].item() else 'False'}" for node in range(len(out_nodes))]
        predictions_output = "\n".join(predictions)
    
        output_line = (identifier if not identifier is None else "") + " " + (" ".join(winning_region) + "\n")
        output.write(output_line)
    
        # Write predictions to separate file
        predictions_file = "node_predictions.txt"  
        with open(predictions_file, "a") as f:
            f.write(f"{identifier if identifier else 'Unknown'}:\n{predictions_output}\n\n")
...
if __name__ == "__main__":
 ...
 files = os.listdir('path/to/games')
 files.sort()
 file_paths = [os.path.join('path/to/games', file) for file in files]
